            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div class="space-y-2">
                        ...
                        <div>
                            ...
                            <input id="name" name="name" type="text"
                                value="{{ old('name', $employee->name) }}" placeholder="Masukkan nama"
                                class="mt-1 p-2 w-full bg-white text-sm border border-gray-200 rounded-md shadow-sm focus:outline-black">
                            @error('name')
                                <div class="text-red-500 text-xs mt-1">{{ $message }}</div>
                            @enderror
                ...
                <div>
                    <div class="space-y-2">
                        ...
                        <div x-data="{
                            imagePreview: null,
                            oldImage: '{{ $employee->avatar ? asset('storage/' . $employee->avatar) : asset('img/placeholder-profile.webp') }}'
                        }">
                            ...
                            <input id="avatar" type="file" name="avatar" accept="image/jpeg, image/png"
                                ...
                                reader.onload = (e) => { imagePreview = e.target.result; oldImage = null; }; 
                                reader.readAsDataURL(file); 
                            } else { imagePreview = null; }">
                            <!-- Preview Gambar -->
                            <div class="mt-4">
                                <div class="overflow-auto max-w-full h-64 rounded-md border border-gray-300">
                                    <!-- Menampilkan gambar lama jika tidak ada gambar baru -->
                                    <template x-if="!imagePreview && oldImage">
                                        <img :src="oldImage" class="w-full h-auto" alt="Current Image">
                                    </template>
                                    <!-- Menampilkan preview jika ada gambar baru -->
                    ...
                    <div class="space-y-2">
                        ...
                        <div>
                            ...
                            <input id="email" name="email" type="email"
                                value="{{ old('email', $employee->user->email) }}" placeholder="Masukkan email"
                                class="mt-1 p-2 w-full bg-white text-sm border border-gray-200 rounded-md shadow-sm focus:outline-black">
                            @error('email')
                                <div class="text-red-500 text-xs mt-1">{{ $message }}</div>
                            @enderror
