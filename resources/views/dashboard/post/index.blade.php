            ...
            <table class="min-w-full bg-gray-100">
                <thead class="bg-[#141652] text-white">
                    <tr>
                        ...
                        <th class="text-left py-3 px-2 text-sm font-semibold">Judul</th>
                        <th class="text-left py-3 px-2 text-sm font-semibold">Penulis</th>
                ...
                <tbody>
                    @forelse ($posts as $post)
                        <tr class="border-t hover:bg-blue-100 transition duration-200">
                            ...
                            <td class="py-3 px-2">{{ Str::limit($post->title, 20) }}</td>
                            <td class="py-3 px-2">{{ Str::limit($post->author->employee->name, 20) }}</td>
                            <td class="py-3 px-2 text-center">
                                <span
                                    class="inline-block px-3 py-1 rounded-full text-xs shadow-md {{ $post->status === 'Terbit' ? 'bg-green-600 hover:bg-green-500' : 'bg-orange-600 hover:bg-orange-500' }} text-white">
                                    {{ $post->status === 'Terbit' ? 'Terbit' : 'Arsip' }}
                                </span>
                            </td>
                            <td class="py-3 px-2 text-center">
                                {{ \Carbon\Carbon::parse($post->created_at)->translatedFormat('d M Y') }}</td>
                            <td class="py-3 px-2 flex items-center justify-center gap-1">
                                ...
                                @else
                                    <button type="button" onclick="alert('Tidak dapat mengubah berita orang lain');"
                                        aria-label="Tombol edit"
                                        class="bg-yellow-600 hover:bg-yellow-500 opacity-60 text-white p-1 rounded-md text-xs shadow-md">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                            stroke-width="1.5" stroke="currentColor" class="size-5">
                                            <path stroke-linecap="round" stroke-linejoin="round"
                                                d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0 1 15.75 21H5.25A2.25 2.25 0 0 1 3 18.75V8.25A2.25 2.25 0 0 1 5.25 6H10" />
                                ...
                                @can('delete-post', $post)
                                    <form action="{{ route('dashboard.posts.destroy', $post->slug) }}" method="POST"
                                        ...
                                        @method('DELETE')
                                        <button
                                            class="flex justify-center items-center bg-red-600 hover:bg-red-500 text-white p-1 rounded-md text-xs shadow-md">
                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                                stroke-width="1.5" stroke="currentColor" class="size-5">
                                                <path stroke-linecap="round" stroke-linejoin="round"
                                ...
                                @else
                                    <button type="button" onclick="alert('Tidak dapat menghapus berita orang lain');"
                                        aria-label="Tombol edit"
                                        class="flex justify-center items-center bg-red-600 hover:bg-red-500 opacity-60 text-white p-1 rounded-md text-xs shadow-md">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                            stroke-width="1.5" stroke="currentColor" class="size-5">
                                            <path stroke-linecap="round" stroke-linejoin="round"
                                                d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0" />
