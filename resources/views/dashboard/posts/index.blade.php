                ...
                <div class="flex flex-col lg:flex-row items-center justify-between gap-4">
                    ...
                    <form method="GET" action="{{ route('dashboard.post.index') }}" class="w-full flex justify-end gap-1"
                        ...
                        <div class="relative">
                            ...
                            <div x-show="openFilter === 'status'" @click.away="openFilter = ''" x-transition
                                ...
                                <select name="status" x-on:change="$root.submit()"
                                    ...
                                    <option value="Arsip" @selected(request('status') === 'Arsip')>Arsip
                                    </option>
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="w-full lg:w-80">
                            <x-forms.input type="text" name="search" placeholder="Cari judul atau penulis berita..."
            ...
            <table class="min-w-full text-sm">
                <thead class="bg-[#141652] text-gray-50">
                    <tr>
                        ...
                        <th class="p-2 font-normal text-left border-r border-gray-600">Ditulis oleh</th>
                    ...
                    @forelse($posts as $post)
                        <tr class="hover:bg-blue-50">
                            <td class="p-2 text-center border-r border-gray-300">
                                {{ ($posts->currentPage() - 1) * $posts->perPage() + $loop->iteration }}</td>
                            <td class="p-2 border-r border-gray-300 whitespace-nowrap">
                                {{ Str::limit($post->title, 25) }}</td>
                            <td class="p-2 border-r border-gray-300 whitespace-nowrap">
                                {{ Str::limit($post->author?->employee->name, 20) }}</td>
