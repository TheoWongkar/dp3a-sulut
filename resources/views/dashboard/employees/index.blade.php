                    ...
                    <form method="GET" action="{{ route('dashboard.employee.index') }}"
                        ...
                        <div class="relative">
                            ...
                            <div x-show="openFilter === 'status'" @click.away="openFilter = ''" x-transition
                                ...
                                <label class="block text-xs text-gray-500 mb-1">Status Pegawai</label>
                                <select name="status" x-on:change="$root.submit()"
                                    ...
                                    <option value="Pensiun" @selected(request('status') === 'Pensiun')>Pensiun</option>
                                    <option value="Meninggal Dunia" @selected(request('status') === 'Meninggal Dunia')>Meninggal Dunia
                                    </option>
                                    <option value="Diberhentikan" @selected(request('status') === 'Diberhentikan')>Diberhentikan</option>
                                </select>
                            </div>
                        </div>
            ...
            <table class="min-w-full text-sm">
                <thead class="bg-[#141652] text-gray-50">
                    <tr>
                        ...
                        <th class="p-2 font-normal text-center border-r border-gray-600">Berita</th>
                        <th class="p-2 font-normal text-center border-r border-gray-600">Laporan</th>
                ...
                <tbody class="divide-y divide-gray-300">
                    @forelse($employees as $employee)
                        <tr class="hover:bg-blue-50">
                            ...
                            <td class="p-2 border-r border-gray-300 whitespace-nowrap">{{ $employee->position }}</td>
                            <td class="px-1 py-2 text-center border-r border-gray-300 whitespace-nowrap">
                                {{ $employee->user?->posts->count() ?? 0 }}
                            </td>
                            <td class="px-1 py-2 text-center border-r border-gray-300 whitespace-nowrap">
                                {{ $employee->user?->statusChanges->count() ?? 0 }}
                            </td>
                            <td class="p-2 text-center border-r border-gray-300">
                                @php
                    ...
                    @empty
                        <tr>
                            <td colspan="10" class="p-4 text-center text-gray-500">Tidak ada data karyawan</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
